...
def convert_examples_to_features(js,tokenizer,args):
    ...
    for i in range(len(js['fuzz'])):
        ...
        if len(tmp) < args.block_size - len(source_tokens):
            ...
            results.append(InputFeatures(source_tokens_f,source_ids_f,js['index'],int(js['label']), type_ids, pos_ids))
    ...
    # whether use raw data w/o fuzz data
    ...
    if len(results) == 0:
        ...
        results.append(InputFeatures(source_tokens,source_ids,js['index'],int(js['label']), type_ids, pos_ids))
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):   
        ...
        return (torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].type_ids), torch.tensor(self.examples[i].pos_ids), \
                torch.tensor(p_example.input_ids), torch.tensor(p_example.type_ids), torch.tensor(p_example.pos_ids), \
                torch.tensor(n_example.input_ids), torch.tensor(n_example.type_ids), torch.tensor(n_example.pos_ids), \
                torch.tensor(label))
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        for step, batch in enumerate(bar):
            ...
            p_inputs = batch[3].to(args.device)
            p_inputs_type_ids = batch[4].to(args.device) 
            p_inputs_pos_ids = batch[5].to(args.device) 
            
            n_inputs = batch[6].to(args.device)
            n_inputs_type_ids = batch[7].to(args.device) 
            n_inputs_pos_ids = batch[8].to(args.device) 
            
            label = batch[9].to(args.device)
            model.train()
            
            loss,vec = model(inputs, inputs_type_ids, inputs_pos_ids, 
                             p_inputs, p_inputs_type_ids, p_inputs_pos_ids, 
                             n_inputs, n_inputs_type_ids, n_inputs_pos_ids,
                             label)
...
def evaluate(args, model, tokenizer,eval_when_training=False):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            lm_loss, vec = model(inputs, inputs_type_ids, inputs_pos_ids, 
                             p_inputs, p_inputs_type_ids, p_inputs_pos_ids, 
                             n_inputs, n_inputs_type_ids, n_inputs_pos_ids,
                             label)
    ...
    for i in range(len(scores)):
        ind = np.argpartition(scores[i], -args.n_candidate)[-args.n_candidate:]
        topk = ind[np.argsort(scores[i][ind])[::-1]]
        sort_ids.append(topk)
    sort_ids = np.array(sort_ids)
...
def test(args, model, tokenizer):
    ...
    with open(os.path.join(args.output_dir,"predictions.jsonl"),'w') as f:
        for index,sort_id in zip(indexs,sort_ids):
            ...
            js['index']=index
            js['answers']=[]
            for idx in sort_id[:args.n_candidate]:
...
def main():
    ...
    parser.add_argument("--unite_crash_hang", action='store_true', help="Whether to unite the crash and hang token")
    ...
    parser.add_argument('--n_candidate', type=int, default=499,
                        help="number of solutions belonging to the same problem except for self")
